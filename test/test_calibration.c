...
static esp_foc_calibration_data_t make_payload(float kp, float ki, float kd,
                                               float kff, float lim)
{
    esp_foc_calibration_data_t d = {0};
    d.kp = q16_from_float(kp);
    d.ki = q16_from_float(ki);
    d.kd = q16_from_float(kd);
    d.kff = q16_from_float(kff);
          ...
          "[espFoC][calibration]")
{
    ...
    esp_foc_calibration_data_t in = make_payload(
        1.234f, 567.89f, 0.01f, 0.99f, 12.0f);
    ...
    TEST_ASSERT_EQUAL_INT32(in.kd, out.kd);
    TEST_ASSERT_EQUAL_INT32(in.kff, out.kff);
          ...
          "[espFoC][calibration]")
{
    ...
    esp_foc_calibration_data_t a = make_payload(1.0f, 100.0f, 0.0f, 1.0f, 1.0f);
    esp_foc_calibration_data_t b = make_payload(2.0f, 200.0f, 0.0f, 1.0f, 2.0f);
...
TEST_CASE("calibration: erase wipes every axis",
          "[espFoC][calibration]")
{
    esp_foc_calibration_data_t d = make_payload(1.0f, 1.0f, 0.0f, 1.0f, 1.0f);
