/*
 * Unit tests for esp_foc_encoder_simu (open-loop motor + encoder mimic).
 */
#include "sdkconfig.h"
...
TEST_CASE("esp_foc_encoder_simu: CPR is 4096", "[espFoC][rotor_simu]")
{
    q16_t vdc = q16_from_float(48.0f);
    esp_foc_encoder_t *s =
        esp_foc_encoder_simu_new(0, 4, 1.0f, 0.002f, vdc);
    TEST_ASSERT_NOT_NULL(s);
    TEST_ASSERT_EQUAL(4096u, s->get_counts_per_revolution(s));
}

TEST_CASE("esp_foc_encoder_simu: unwired u stays at zero angle", "[espFoC][rotor_simu]")
{
    q16_t vdc = q16_from_float(48.0f);
    esp_foc_encoder_t *s =
        esp_foc_encoder_simu_new(0, 4, 1.0f, 0.002f, vdc);
    TEST_ASSERT_NOT_NULL(s);
    esp_foc_encoder_simu_wire_ud_uq(s, NULL, NULL);
    for (int i = 0; i < 50; ++i) {
...
TEST_CASE("esp_foc_encoder_simu: wired uq produces motion", "[espFoC][rotor_simu]")
{
    q16_t vdc = q16_from_float(48.0f);
    esp_foc_encoder_t *s =
        esp_foc_encoder_simu_new(0, 4, 0.5f, 0.001f, vdc);
    TEST_ASSERT_NOT_NULL(s);
    q16_t ud = q16_from_float(0.0f);
    q16_t uq = q16_from_float(0.15f);
    esp_foc_encoder_simu_wire_ud_uq(s, &ud, &uq);
...
TEST_CASE("esp_foc_encoder_simu: set_to_zero clears state", "[espFoC][rotor_simu]")
{
    q16_t vdc = q16_from_float(48.0f);
    esp_foc_encoder_t *s =
        esp_foc_encoder_simu_new(0, 4, 0.5f, 0.001f, vdc);
...
TEST_CASE("esp_foc_encoder_simu: accumulated tracks unwrap", "[espFoC][rotor_simu]")
{
    q16_t vdc = q16_from_float(48.0f);
    esp_foc_encoder_t *s =
        esp_foc_encoder_simu_new(0, 4, 0.5f, 0.001f, vdc);
