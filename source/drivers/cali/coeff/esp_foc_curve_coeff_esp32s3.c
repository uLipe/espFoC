 ...
 *
 * Curve-fitting coefficients ported from ESP-IDF esp_adc/esp32s3/curve_fitting_coefficients.c
 */
...
void esp_foc_curve_fitting_get_second_step_coeff(adc_unit_t unit, adc_atten_t atten, esp_foc_cali_chars_second_step_t *ctx)
{
    ...
    ctx->coeff = unit == ADC_UNIT_1 ?
                 adc1_error_coef_atten[atten] :
                 adc2_error_coef_atten[atten];
    ctx->sign = unit == ADC_UNIT_1 ?
                adc1_error_sign[atten] :
                adc2_error_sign[atten];
}
