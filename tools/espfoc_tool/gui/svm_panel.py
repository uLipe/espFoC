...
Reads scope channels 10/11/12 (iu, iv, iα) from axis_tuning firmware map
and shows:
...
class SvmPanel(QWidget):
    ...
    def __init__(self, reader: Optional[LinkReader] = None,
        ...
        self._uu_curve = self._wave_plot.plot(
            pen=pg.mkPen(QColor(_PHASE_COLORS[0]), width=2), name="iu (ch10)")
        self._uv_curve = self._wave_plot.plot(
            pen=pg.mkPen(QColor(_PHASE_COLORS[1]), width=2), name="iv (ch11)")
    ...
    def _render_tick(self) -> None:
        ...
        for _t_mono, payload in pending:
            ...
            if len(vals) < 13:
                return
            iu, iv, ialpha = vals[10], vals[11], vals[12]
            ibeta = -(iu + iv) * 0.57735026919
            u_u, u_v, u_w = iu, iv, -(iu + iv)
            _ = ialpha, ibeta
