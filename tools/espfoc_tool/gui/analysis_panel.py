...
class AnalysisPanel(QWidget):
    ...
    def _on_apply_mpz(self) -> None:
        if self._client is None:
            return
        try:
            motor = MotorParams(
                r_ohm=self._last_r, l_h=self._last_l,
                ts_s=self._ts_s(), v_max=12.0)
            gains = mpz_design(motor, self._last_bw)
            self._client.write_kp(gains.kp)
    ...
    def update_model(self, motor_r: float, motor_l: float, bw_hz: float,
        ...
        self._last_r = motor_r
        self._last_l = motor_l
        self._last_bw = bw_hz
        try:
