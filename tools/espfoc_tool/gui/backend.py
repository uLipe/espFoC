...
from .plot_util import decimate_xy
...
class ScopeBackend(QObject):
    ...
    def _set_status(self, text: str) -> None:
        ...
        self._status_text = f"{text}  |  frames={frames}  seq_gaps={gaps}"
    ...
    def refresh_plots(self, window_s: float = 5.0) -> None:
        ...
        for row in range(self._plot_model.rowCount()):
            ...
            if window_s > 0 and x.size > 1:
                t_max = x[-1]
                mask = x >= (t_max - window_s)
                x = x[mask]
                y = y[mask]
            if x.size > 2048:
                x, y = decimate_xy(x, y, 2048)
