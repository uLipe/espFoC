#!/usr/bin/env python3
"""espfocctl — interactive espFoC device client (protocol v2).

Opens one serial session, runs connect, then accepts subcommands on stdin.
One-shot: ``espfocctl --port DEV align`` → connect → align → disconnect.
"""
...
class TunerShell:
    ...
    def _cmd_estop(self, _args: list[str]) -> int:
        self.cli.write_target_id(0.0)
        self.cli.write_target_iq(0.0)
        self.cli.stop_axis()
        print("estop: id/iq=0, axis stopped, inverter disabled")
        return 0
    ...
    def repl(self) -> int:
        print("espfocctl interactive — type 'help' or 'quit'")
        while True:
            try:
                line = input("espfoc> ")
...
def _one_shot(cli: TunerClient, cmd: str, cmd_args: list[str]) -> int:
    ...
    try:
        ...
        print(f"espfocctl: connect failed: {e}", file=sys.stderr)
        return 1
    try:
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="espfocctl")
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    try:
        ...
        print(f"espfocctl: {e}", file=sys.stderr)
        return 1
    finally:
        cli.close()
